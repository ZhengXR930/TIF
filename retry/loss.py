...
# ——— 数据生成函数 ———
def gen_data(bs=256, D=1000):
    x = torch.randint(0,2,(bs,D),dtype=torch.float32)
    y = (x[:,:50].sum(dim=1) > 25).long()
    return x, y
...
if __name__ == "__main__":
    ...
    if model.proj is not None:
        optimizer_params.append({'params': model.proj.parameters(), 'lr': 1e-4})
    optimizer_params.append({'params': model.proxies, 'lr': 1e-4})
    
    optimizer = torch.optim.Adam(optimizer_params)
    
    print("Step\tTotalLoss")
    for step in range(1, 51):
        xb, yb = gen_data(256, 1000)
