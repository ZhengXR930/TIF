...
Results are saved as CSV files in the `result_folder`:
- `stage1_test_file.csv`: Stage 1 results
- `stage2_test_file_{seed}.csv`: Stage 2 results
- `tif_test_file.csv`: Full TIF results
- `svm_test_file.csv`: SVM baseline results
- `deep_test_file.csv`: DeepDrebin results
- `ts_test_file_{seed}.csv`: T-Stability results
- `mpc_test_file.csv`: MPC results

Each CSV file contains columns: `month,precision,recall,f1,aut` (where `aut` is the Area Under Time curve metric from tesseract).

## Notes

- IRM (Invariant Risk Minimization) methods are sensitive to hyperparameter selection
- Results may vary based on random seed
- Ensure sufficient GPU memory for training (especially for larger batch sizes)
- The code uses deterministic settings when seed is set for reproducibility
- Logs are saved to `logs/` directory when using `run_experiment.sh`
