...
class UncertaintySamplingSelector(Selector):
    def __init__(self, n, schedule=1, tracking=True,
                 interaction='intersection', DNN=0): #todo controllare
        super().__init__(schedule, tracking, interaction)
        self.n = n
        self.DNN=DNN
    ...
    def query(self, clf, X_train, y_train, t_train,
        ...
        # e.g. clf is a DNN with Softmax prediction
        elif hasattr(clf, 'predict') and (self.DNN==1):
            y_probs = clf.predict(X_test) #TODO vale solo per NN
            #TODO aggiungere scelta tra soglia e percentuale
            selected_indexes = probabilistic_uncertainty(y_probs, m)
        else:
            raise TypeError(
...
class RandomSamplingSelector(Selector):
    def __init__(self, n, schedule=1, tracking=True,
    ...
    def query(self, clf, X_train, y_train, t_train,
        ...
        selected_indexes=np.random.choice(X_test.shape[0], m, replace=True) 
