...
def get_train_info(X_train, y_train, t_train, existing=None, positive_label=0):
    ...
    if positive_label==1:
       ...
       results['train_neg'].append(train_pos)
       results['train_pos'].append(len(y_train) - train_pos)
       results['train_tot'].append(len(y_train))
...
def calculate_metrics(y_true, y_pred, existing=None,
                      raw_scores=None, periods=1, positive_label=0):
    p =positive_label
    periods = len(y_pred) if periods == -1 else periods

    if periods > 1:
        for y_t, y_p in zip(y_true, y_pred):
            existing = calculate_metrics(y_t, y_p, existing, raw_scores, positive_label=p)
        return existing
    ...
    if len(y_pred) > 1: #to ensure that there are samples in the window
        ...
        #Passing the label of positive class
        if positive_label==0:
           print('Positive label: 0')
           tp, fn, fp, tn = confusion_matrix(y_true, y_pred, labels=(0, 1)).ravel()
        else:
        ...
        results['f1'].append(skmetrics.f1_score(y_true, y_pred, pos_label=0))
        ...
        results['f1_n'].append(skmetrics.f1_score(y_true, y_pred, pos_label=1))

        results['OA'].append(skmetrics.accuracy_score(y_true, y_pred))
    else:
        results['tp'].append(0)
        results['fp'].append(0)
        results['tn'].append(0)
        ...
        results['precision_n'].append(np.nan)
        results['recall_n'].append(np.nan)
        results['f1_n'].append(np.nan)

        results['OA'].append(np.nan)
