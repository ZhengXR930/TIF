...
"""
import sys
sys.path.append('../INSOMNIA/INSOMNIA')
import bisect
import operator
...
def time_aware_train_test_split_with_family(X, y, y_family,t, train_size, test_size,
                                granularity, start_date=None):
    """Partition a dataset composed of time-labelled objects.
    ...
    Args:
        X (np.ndarray): Multi-dimensional array of predictors.
        y (np.ndarray): Array of output labels.
        y_family (np.ndarray): Family label of malware
        t (np.ndarray): Array of timestamp tags.
        train_size (int): The training window size W (in τ).
        test_size (int): The testing window size Δ (in τ).
        granularity (str): The unit of time τ, used to denote the window size.
    ...
    X_actual, y_actual, y_family_actual, t_actual = X[train], y[train], y_family[train],t[train]

    X_tests = [X[index_set] for index_set in tests]
    y_tests = [y[index_set] for index_set in tests]
    y_family_tests = [y_family[index_set] for index_set in tests]
    t_tests = [t[index_set] for index_set in tests]

    return X_actual, X_tests, y_actual, y_tests,y_family_actual,y_family_tests, t_actual, t_tests
...
def get_relative_delta(offset, granularity):
    ...
    try:
        return {
            ...
            'hour': relativedelta(hours=offset)
        }[granularity]
    except KeyError:
        raise ValueError('granularity not recognised, try: '
                         'year|quarter|month|week|day|hours')
                         

#New
def count_train_test_split(X, y, t, train_size, test_size):
    """Partition a dataset with fixed windows.

    Args:
        X (np.ndarray): Multi-dimensional array of predictors.
        y (np.ndarray): Array of output labels.
        t (np.ndarray): Array of timestamp tags.
        train_size (int): The training window size W (integer).
        test_size (int): The testing window size Δ (integer).
    ...
    """
    to_idx = train_size
    X_actual, y_actual, t_actual = X[:to_idx], y[:to_idx], t[:to_idx]

    X_tests=[]
    y_tests=[]
    t_tests=[]
    # Slice out testing partitions
    while to_idx < len(X)+1:
        from_idx = to_idx
        to_idx=from_idx+test_size
        #print('from:' + str(from_idx) + ' to:' + str(to_idx))
        X_tests.append(X[from_idx:to_idx])
        y_tests.append(y[from_idx:to_idx])
        t_tests.append(t[from_idx:to_idx])
