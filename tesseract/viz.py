...
def plot_decay(results, fill=True, titles=None, means=None):
    ...
    for i in range(len(results)):
        ...
        results[i] = pd.DataFrame(dict(results[i]),
                                  index=range(1, len(results[i]['f1'])+1)) #remove +1 error broadcast
...
def add_legend(ax, loc='lower left'):
    lines = ax.get_lines()
    legend=ax.legend(loc='lower center', bbox_to_anchor=(0.45, -0.26),
              fancybox=True, shadow=True, ncol=3, fontsize=7,handles=lines)
